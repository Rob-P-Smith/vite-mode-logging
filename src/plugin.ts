 ...
 */
function ensureModeFunction(mode: string, projectRoot: string) {
  const debugFilePath = path.resolve(projectRoot, 'src/debug.ts')
...
`
    ...
    console.log(`[vite-mode-logging] Added ${functionName} function to debug.ts`)
  }
  ...
  if (!allGlobalRegex.test(content)) {
    ...
    fs.writeFileSync(debugFilePath, content, 'utf-8')
    console.log('[vite-mode-logging] Added ALL function to debug.ts')
  }
}
 ...
 */
export function modeBasedLoggingPlugin(mode: string, options?: { projectRoot?: string }): Plugin {
  const projectRoot = options?.projectRoot || process.cwd()
  
  // Ensure the mode function exists in debug.ts
  ensureModeFunction(mode, projectRoot)
  
  return {
    name: 'vite-mode-logging',
    enforce: 'pre',
